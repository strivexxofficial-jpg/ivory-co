...
async function startServer() {
  ...
  app.post("/api/smile-sim-process", async (req, res) => {
    try {
      // Placeholder for hybrid pipeline: Detection -> Segmentation -> Warping -> Blending -> Subtle AI Refinement
  ...
  async function uploadToImgBB(base64Data: string): Promise<string> {
    ...
    formData.append('expiration', '600'); // auto-delete after 10 mins
  ...
  app.post("/api/pixazo", async (req, res) => {
    try {
      ...
      const response = await fetch("https://gateway.pixazo.ai/inpainting/v1/getImage", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          prompt: "Bright white clean teeth. Professional teeth whitening. Remove all yellow stains and discoloration. Make teeth visibly white and clean.",
          negativePrompt: "yellow teeth, stained teeth, discolored enamel, dark spots, plaque",
      ...
      catch (e) { return res.status(response.status).send(text); }
  ...
  app.post("/api/pixazo/poll", async (req, res) => {
    try {
      const response = await fetch("https://gateway.pixazo.ai/ai-model-api-polling/getGenerationResults", {
        method: "POST",
        headers: {
          "Ocp-Apim-Subscription-Key": process.env.PIXAZO_KEY || "",
          "Content-Type": "application/json"
